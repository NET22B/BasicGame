...
namespace BasicGame.ConsoleGame.Extensions
{
    ...
    public static class ExtensionTestFunc
    {
        public static Func<IConfiguration, string, int> Implementation { private get; set; } = // = Test; 
        (config, value) =>
        {
            var section = config.GetSection("game:mapsettings");
            return int.TryParse(section[value], out int result) ? result : 0;
    ...
    public static int GetMapSizeForFunc(this IConfiguration config, string value)
        {
            return Implementation(config, value);
        }

        //public static int Test(IConfiguration config, string value)
        //{
        //    var section = config.GetSection("game:mapsettings");
        //    return int.TryParse(section[value], out int result) ? result : 0;
        //}
